...
int main() {
    ...
    while (true) {
        ...
        if (len < 1200) {
            std::cout << "❌ Packet too small (expected ~1248 bytes), might be fragmented\n";
            continue;
        }
        
        // Skip UDP header (42 bytes) and start parsing data blocks
        const uint8_t* data_start = buffer + 42;
